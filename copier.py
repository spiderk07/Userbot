...
from pyrogram.errors import FloodWait, PeerFlood, UserDeactivatedBan, AuthKeyUnregistered
...
class Copier:
    ...
    async def copy_message(self, client, file_info, account_name=None):
        ...
        for attempt in range(self.retry_limit):
            try:
                ...
                await asyncio.to_thread(self.db.record_floodwait, account_name or 'unknown', e.value)
                await asyncio.sleep(e.value)
                continue

            except (PeerFlood, UserDeactivatedBan, AuthKeyUnregistered) as e:
                # Ye Telegram ka SABSE serious warning hai — matlab account
                # spam/automation jaisa detect ho raha hai (PeerFlood) ya
                # already restrict/ban ho chuka hai. Isme blind retry karna
                # account ko aur zyada risk me daalta hai. Isliye yahan
                # dobara try NAHI karte — seedha upar (worker) ko batate hain
                # taaki wo is poore account ka kaam turant rok de.
                logger.critical(
                    f"⚠️ ACCOUNT RISK ({type(e).__name__}) on message "
                    f"{source_message_id} — is account ka copying rok raha "
                    f"hoon. Manually check karo (naya account use karna pad "
                    f"sakta hai, ya kuch ghanton ka break dena hoga)."
                )
                raise
