    ...
    def _register_handlers(self):
        ...
        async def stats_command(client, message):
            ...
            stats_text = (
                ...
                f"📦 In-memory buffer: `{self.queue_manager.qsize()}`\n"
                f"📬 Mongo queue (queued/processing): `{stats['queue_queued']}` / `{stats['queue_processing']}`\n"
                f"🔒 Held (new msgs waiting for backlog): `{stats['queue_live_pending']}`\n\n"
            )
            ...
            diag = await asyncio.to_thread(self.db.get_diagnostics)
            if diag:
                stats_text += "\n**Diagnostics:**\n"
        ...
        async def progress_command(client, message):
            ...
            progresses = await asyncio.to_thread(self.db.get_all_progress)
            if not progresses:
