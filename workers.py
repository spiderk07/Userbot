...
from pyrogram.errors import PeerFlood, UserDeactivatedBan, AuthKeyUnregistered
import logging
import random
...
class Worker:
    ...
    async def process_queue(self):
        while self.is_running:
            ...
            except (PeerFlood, UserDeactivatedBan, AuthKeyUnregistered) as e:
                # Account-level serious risk — is account ka kaam yahin
                # permanently rok do (baaki accounts/workers chalte rahenge).
                # File abhi 'processing' status me hi reh jayegi — koi
                # mark_failed/completed nahi karte — 3 minute baad
                # recover_stuck_processing() ise khud 'queued' bana degi
                # taaki koi DOOSRA sahi-salamat account use pick kar sake.
                logger.critical(
                    f"🛑 Worker {self.worker_id} ({self.account_name}) STOPPED "
                    f"— account risk detected ({type(e).__name__}). Isko "
                    f"restart se dobara try nahi karna — pehle account ki "
                    f"health manually check karo Telegram app khol ke."
                )
                await asyncio.to_thread(
                    self.copier.db.record_worker_stopped,
                    self.worker_id, self.account_name, type(e).__name__
                )
                self.is_running = False
                return
    ...
    async def run(self):
        # Config.WORKER_CONCURRENCY se decide hota hai ki ye account ek
        # saath kitni "lanes" chalaye. Har lane apna alag process_queue()
        # loop hai, sab same Client aur QueueManager share karte hain
        # (claim_batch Mongo-atomic hai, isliye do lanes kabhi same file
        # claim nahi karenge). Default 1 = purana sequential behavior.
        lanes = max(1, Config.WORKER_CONCURRENCY)
        await asyncio.gather(*[self.process_queue() for _ in range(lanes)])
...
class WorkerPool:
    ...
    async def start_all(self, clients):
        ...
        if clients:
            # Purani deployment ke stale worker entries (jaise pehle 4
            # sessions the, ab 2 hain) clear karo — warna /stats hamesha
            # purane account count dikhata rahega.
            await asyncio.to_thread(Database().reset_workers)
        for i, client in enumerate(clients, 1):
            ...
            await asyncio.to_thread(worker.queue_manager.db.record_worker_started, i, account_name)
